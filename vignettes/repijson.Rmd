...
#convert the text dates to date-time objects
#note the use of POSIXct, this is more friendly to dataframes
exampledata2$rec1date <- as.POSIXct(exampledata2$rec1date)
exampledata2$rec2date <- as.POSIXct(exampledata2$rec2date)
exampledata2$dob <- as.POSIXct(exampledata2$dob)

eg2 <- as.ejObject(exampledata2, recordAttributes = list(oldID="id", name="name", "dob", "gender"),
     eventDefinitions = list(define_ejEvent(name="Contact1", date="rec1date", attributes=list("rec1contact", "rec1risk","rec1temp")),
                             define_ejEvent(name="Contact2", date="rec2date", attributes=list("rec2contact", "rec2risk"))),
 		metadata=list())
eg2
```
