...
#' 
#' @examples
#' #commenting out example
#' #i think example fails check because the function isn't exported
...
dataFrameToAttributes <- function(x){
	...
	result <- apply(expand.grid(i=1:nrow(x), j=1:ncol(x)), 1, function(attpos){
				create_ejAttribute(attributeNames[attpos[2]], attributeTypes[attpos[2]], x[attpos[1],attpos[2]])
			})
	return(result)
}
